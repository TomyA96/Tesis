...
const variantStyles: Record<Variant, string> = {
  ...
  warning:   "bg-amber-100 text-amber-700 hover:bg-amber-300 focus:ring-amber-400",
...
// Compartida con LinkBtn, para que un link estilizado como botón (ej. "Ver
// Ventas") nunca pueda desincronizarse visualmente de los botones reales.
export const claseBtn = (variant: Variant = "primary", size: Size = "md", className = "") =>
  cn(baseStyles, variantStyles[variant], sizeStyles[size], className);

const Btn = ({ variant = "primary", size = "md", className = "", ...props }: BtnProps) => {
  return (
    <button
      className={claseBtn(variant, size, className)}
      {...props}
    />
  );
};
